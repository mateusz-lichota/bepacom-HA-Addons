...
from typing import Annotated, Any, Callable, Mapping, Protocol, Union, cast
...
bacnet_device_dict: dict[str, Any]
bacnet_application: BACnetIOHandler
activeSockets: list[WebSocket] = []
EDE_files: list[DeviceDict] = []
sub_list: list[Task[None]] = []


class WhoIsFunc(Protocol):
    def __call__(
        self,
...
who_is_func: WhoIsFunc
i_am_func: Callable[[], None]
ingress: str
...
def deep_update[K](
    mapping: Mapping[K, Any], *updating_mappings: Mapping[K, Any]
) -> dict[K, Any]:
    """"""
...
class EventStruct:
    ...
    write_queue: Queue[WriteQueueElem] = Queue()
    sub_queue: Queue[SubQueueElem] = Queue()
    unsub_queue: Queue[UnsubQueueElem] = Queue()
    val_updated_event: Event = Event()
    read_event: Event = Event()
    who_is_event: Event = Event()
    i_am_event: Event = Event()
    startup_complete_event: Event = Event()
...
async def upload_ede_files(
    response: Response, EDE: UploadFile, stateTexts: UploadFile | None = None
):
    """Upload EDE files to show up as placeholder in the object lists."""

    deviceDict = {}
    liststart = False
    stateTextsList: list[list[str]] = []
    statecounter = 0
    ...
    for row in csvEDE:
        if liststart:
            ...
            if stateTextsList and "binary" in str(obj_type.attr):
                obj_dict |= {
                    "inactiveText": stateTextsList[int(state_text)][0],
                    "activeText": stateTextsList[int(state_text)][1],
                }
            ...
            deviceDict: DeviceDict = deep_update(
                deviceDict,
                {
...
async def write_property_v1(
    ...
    try:
        ...
        if outOfService is not None:
            property_dict.update({"outOfService": outOfService})
        if covIncrement is not None:
        ...
        if property_dict:
            for key, val in property_dict.items():
                await events.write_queue.put(
                    WriteQueueElem(
                        device_id=ObjectIdentifier(deviceid),
                        object_id=ObjectIdentifier(objectid),
                        property_id=PropertyIdentifier(key),
                        property_val=val,
                        array_index=None,
                        priority=None,
                    )
...
async def write_property_v2(
    ...
    def as_bool(input_val: Any) -> bool:
        if isinstance(input_val, bool):
            return input_val
        if isinstance(input_val, str):
            match input_val.lower():
                case "true":
                    return True
                case "false":
                    return False
                case _:
                    pass
        raise Exception(f"Not a bool: {input_val} ({type(input_val)})")
    ...
    try:
        ...
        if not address:
            return JSONResponse(
                content=jsonable_encoder({"result": "Unknown device"}), status_code=400
            )
        ...
        return JSONResponse(
            content=jsonable_encoder({"result": str(err)}), status_code=400
        )
    ...
    try:
        ...
        return JSONResponse(
            content=jsonable_encoder({"result": str(err)}), status_code=405
        )
